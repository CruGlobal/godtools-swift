//
//  FakeMobileContentAuthTokenApi.swift
//  godtoolsTests
//
...
final class FakeMobileContentAuthTokenApi: AuthTokenApiInterface {
    ...
    func fetchAuthToken(providerToken: MobileContentAuthProviderToken, createUser: Bool) async throws -> Result<MobileContentAuthTokenCodable, MobileContentApiError> {
        ...
        guard let token = fetchedAuthToken else {
            return .failure(MobileContentApiError.other(error: NSError(domain: "TestError", code: -2, userInfo: [NSLocalizedDescriptionKey: "No token configured"])))
        }
