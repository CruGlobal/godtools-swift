...
class GetToolSettingsPrimaryLanguageRepository: GetToolSettingsPrimaryLanguageRepositoryInterface {
    ...
    @MainActor func getLanguagePublisher(primaryLanguageId: String, translateInLanguage: AppLanguageDomainModel) -> AnyPublisher<ToolSettingsToolLanguageDomainModel?, Never> {
        
        guard let language = languagesRepository.persistence.getDataModelNonThrowing(id: primaryLanguageId) else {
            return Just(nil)
                .eraseToAnyPublisher()
        }
